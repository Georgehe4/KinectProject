...
float DualPlane::confidence(){
	...
	if(a->COG.distancesq(b->COG)>1.f || abs(a->normal.dot(b->normal))<0.866 || abs(a->normal.dot(b->normal))>0.9998	//angle between 1-15deg.
		|| abs((180.f/ML_PI)*a->normal.trad(b->normal))>15){	//15deg.
		return 0;
	}
	float conf = (1.f-(float)(abs(as-bs))/(float)(as+bs));
	conf*=conf;
